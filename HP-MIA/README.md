...
As a test case for HP-MIA, two ipynb files provide the implementation on Purchase100. `Train_ref_Purchase.ipynb` provides codes to train the target model/shadow model/reference model. The target model and shadow model are stored in `target_shadow`. 100 well-trained reference models for Purchase100 are stored in `ref_purchase`. `attack_exp_purchase.ipynb` tested Two-stage HP-MIA on Purchaser100.
